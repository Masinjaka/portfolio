  ...
  return (
    <div className="py-5 first:pt-0">
      <h2>
        ...
        >
          <span className="text-xl font-medium tracking-[-0.03em] text-black dark:text-zinc-50">
            {title}{" "}
          ...
          <span
            aria-hidden="true"
            className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-zinc-200 text-zinc-500 transition duration-300 dark:border-zinc-800 dark:text-zinc-400 ${
          ...
          <ul className="mt-4 flex list-none flex-wrap gap-3">
            {items.map((item) => (
              <li key={item.name} className="flex flex-col items-center gap-2">
                ...
                <span className="max-w-[4.5rem] text-center text-xs leading-4 text-zinc-700 dark:text-zinc-300">
                  {item.name}
                </span>
              </li>
            ))}
